...
function for 1010:5C11 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_5c11(mem, *, ds=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:5C11 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5C11
            _a = mem.rw(ds, 0x41AA)
            _b = 0x12
            _t = _a - _b
