...
from skyroads.recovered.func_1010_609f import func_1010_609f
...
def func_1010_3f3b(mem, plat, *, _base=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    ...
    while True:
        ...
        if bb == 3:  # 1010:3F56
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x3F63)
            _o, _c = func_1010_609f(mem, plat, _base=_base + _cost + 4, _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), ax=ax, bp=bp, cx=cx, di=di, ds=ds, dx=dx, es=es, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
