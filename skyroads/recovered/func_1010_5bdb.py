...
function for 1010:5BDB (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_5bdb(mem, plat, *, _base=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:5BDB (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5BDB
            _a = mem.rw(ds, 0x41AA)
            _b = 0x0
            _t = _a - _b
