...
function for 1010:3C52. DO NOT hand-edit; regenerate.
...
def func_1010_3c52(mem, *, _df=0, bp=0, bx=0, di=0, ds=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:3C52 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:3C52
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, bp)
            bp = sp
            ...
            si = _t & 0xFFFF
            bx = (bx & 0xFF00) | ((mem.rb(ss, ((bp + 10) & 0xFFFF))) & 0xFF)
            ax = 0xA000
            es = ax
            ds = mem.rw(ds, 0x3186)
        ...
        if bb == 2:  # 1010:3C82
            ...
            continue
        if bb == 3:  # 1010:3C86
            mem.wb(es, ((di) & 0xFFFF), ((bx & 0xFF)) & 0xFF)
            _cost += 1
            bb = 4
            continue
        if bb == 4:  # 1010:3C89
