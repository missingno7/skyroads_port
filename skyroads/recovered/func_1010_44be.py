...
function for 1010:44BE. DO NOT hand-edit; regenerate.
...
from skyroads.recovered.func_1010_3c52 import func_1010_3c52
...
def func_1010_44be(mem, *, _df=0, bp=0, bx=0, cx=0, di=0, ds=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:44BE (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:44BE
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, bp)
            bp = sp
            ...
            ax = mem.rw(ds, 0x3188)
            dx = mem.rw(ds, 0x318A)
            mem.ww(ds, 0x3184, ax & 0xFFFF)
            mem.ww(ds, 0x3186, (dx) & 0xFFFF)
        ...
        if bb == 5:  # 1010:44FC
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x450B)
            _o, _c = func_1010_3c52(mem, _df=(1 if df else 0), bp=bp, bx=bx, di=di, ds=ds, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
        ...
        if bb == 6:  # 1010:4511
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, _t)
            raise RuntimeError('CPUless: unrecovered call to 1010:3E8A reached -- frontier witness (untested code path)')
        if bb == 7:  # 1010:4522
            di = mem.rw(ss, sp)
            sp = (sp + 2) & 0xFFFF
            si = mem.rw(ss, sp)
