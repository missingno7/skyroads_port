...
`resolve_move` uses (bind it with `skyroads.recovered_native.collision.make_visible`).
...
@oracle_link(
    ...
    # confirmed on a real death. See skyroads.recovered_native.gaps.FallDeathTransition.
    merge_target="skyroads.recovered_native.collision_response (future)",
)
def ship_fell_off(persp_word: int, af1c: int, af2c: int,
    ...
    is known -- see ``skyroads.recovered_native.collision.ship_fell_off``)."""
