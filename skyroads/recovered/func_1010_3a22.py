...
function for 1010:3A22 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_3a22(mem, *, ax=0, bx=0, ds=0, dx=0, es=0, si=0, ss=0):
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:3A22 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:3A22
            di = si
            cx = 0x1D
            _cost += 2
