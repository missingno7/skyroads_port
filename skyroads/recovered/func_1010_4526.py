...
function for 1010:4526. DO NOT hand-edit; regenerate.
...
from skyroads.recovered.func_1010_44be import func_1010_44be
...
def func_1010_4526(mem, *, _df=0, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:4526 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:4526
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, bp)
            bp = sp
        ...
        if bb == 3:  # 1010:453C
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x4555)
            _o, _c = func_1010_44be(mem, _df=(1 if df else 0), bp=bp, bx=bx, cx=cx, di=di, ds=ds, es=es, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
