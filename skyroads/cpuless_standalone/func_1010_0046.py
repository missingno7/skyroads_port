...
function for 1010:0046 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_0046(mem, plat, *, _base=0, _flags_in=2, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:0046 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:0046
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, bp)
            bp = sp
            ...
            bp = mem.rw(ss, sp)
            sp = (sp + 2) & 0xFFFF
            raise RuntimeError('CPUless: runtime-dead exit at 1010:0068 reached -- frontier witness (untested exit path)')
