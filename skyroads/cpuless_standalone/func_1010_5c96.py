...
function for 1010:5C96 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from skyroads.cpuless_standalone.func_1010_5bdb import func_1010_5bdb

_PARITY = tuple((1 - bin(v).count('1') % 2) == 1 for v in range(256))


def func_1010_5c96(mem, plat, *, _base=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:5C96 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5C96
            bx = sp
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, es)
