...
class FallDeathTransition(SkyroadsGap):
    ...
    (``skyroads.handrecovered_native.collision.ship_fell_off`` is true past the `[41C0]`
...
class MovementPhysicsGap(SkyroadsGap):
    ...
    skyroads.handrecovered.physics) -> ``resolve_move`` (``1010:186B``,
    skyroads.handrecovered.movement) with the ``skyroads.handrecovered_native.collision``
    ...
    momentum -- is now ALSO recovered: ``skyroads.handrecovered.dynamics.
    ...
    needs are now RECOVERED too -- ``skyroads.handrecovered.classify`` /
    ``skyroads.handrecovered_native.classify`` (the ``1010:2324-23BF`` block, 682/682 vs VM).
...
class JumpGateGap(SkyroadsGap):
    ...
    skyroads/handrecovered/player.py::update_vertical_velocity's docstring.
    ...
    RECOVERED (``skyroads.handrecovered.classify`` / ``skyroads.handrecovered_native.classify``,
...
class VerticalVelocityGap(SkyroadsGap):
    """SUPERSEDED by ``skyroads.handrecovered.dynamics.step_jump_steer_gravity``.
