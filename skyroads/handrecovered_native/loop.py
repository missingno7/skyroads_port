...
* :func:`native_gameplay_frame` -- the per-frame gameplay update. Commits
  ...
  run_status.md) and then raises a typed gap (skyroads.handrecovered_native.gaps) the
  ...
  :class:`~skyroads.handrecovered_native.gaps.VerticalVelocityGap`), the jump latch, or the
  ...
  :class:`~skyroads.handrecovered_native.gaps.MovementPhysicsGap` is specifically its
...
from skyroads.handrecovered_native.classify import classify_ship
from skyroads.handrecovered_native.collision import make_visible, ship_fell_off
from skyroads.handrecovered_native.gaps import (
    FallDeathTransition,
    JumpGateGap,
    LevelEndTransition,
    MovementPhysicsGap,
...
def native_gameplay_frame(view: GameView) -> None:
    ...
    Raises :class:`~skyroads.handrecovered_native.gaps.SkyroadsGap` (loudly, before
    ...
    ``resolve_move(..., visible=skyroads.handrecovered_native.collision.make_visible(rw))``
...
def native_gameplay_substep(
    ...
    is a boundary (raises :class:`~skyroads.handrecovered_native.gaps.FallDeathTransition`).
    ...
    :class:`~skyroads.handrecovered_native.gaps.MovementPhysicsGap` (the fail-loud, verified
...
#: the gauges show only their empty outlines -- see `skyroads/handrecovered_native/hud.py`.
...
def _classify_transition(view: GameView, exc: Exception) -> str:
    """Map a raised :class:`~skyroads.handrecovered_native.gaps.LevelEndTransition` /
    :class:`~skyroads.handrecovered_native.gaps.FallDeathTransition` to a coarse,
...
class NativeGameplayDriver:
    ...
    * the rare ``1DFA`` effect sub-step (~0.7% of real sub-steps) -- handled
    ...
    def __init__(self, view: GameView, jump_level_gate: int,
        ...
        #: `skyroads.handrecovered_native.sfx` for the id map and the SFX.SND bank loader.
        self.on_sfx = on_sfx
