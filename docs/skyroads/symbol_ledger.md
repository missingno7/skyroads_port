...
passthrough — see run_status for the dead-stack-below-SP nuance). **Clean
recovery:** `skyroads/handrecovered/movement.py::resolve_move` (pure, `@oracle_link`,
`ASM_MATCHED` 1760/1760 via predicate-oracle replay). Description:
...
| Address | Role | Evidence | Status |
|---|---|---|---|
| `5E5A`-`5EF3` | `_ldiv`/`__aNldiv`-style SIGNED 32-bit long DIVIDE (the signed twin of `5D8C`). MSC ABI: `push bp; mov bp,sp`, callee-cleanup `ret 8`; args `[bp+4..5]`=dividend low, `[bp+6..7]`=dividend high, `[bp+8..9]`=divisor low, `[bp+A..B]`=divisor high; signed quotient in `DX:AX`. Takes each operand's magnitude (negating in place, counting negatives in `DI`), runs the unsigned core (32/16 fast path or shift-normalize 32/32, same shapes as `5D8C`), then negates the result iff exactly one operand was negative (`dec di; jnz`). Called from 9 layer-3 render routines. | full disassembly (2026-07-10, fixed-length linear) + strict differential verifier (full-demo, 2,530 calls, 0 divergence) | VERIFIED (non-negative 32/16 path); negative-operand + 32/32 paths DELEGATED to ASM |
...
**Hooked as `signed_long_div`** (`skyroads/hooks.py`) — lifts only the all-non-negative 32/16 case (every call in the recorded demo) exactly (DX:AX = |num|//|den|; CX left = divisor_lo; final flags from the sign-fixup `dec di` 0->0xFFFF with CF=0), and delegates any negative operand or 32/32 divisor to the original ASM via `interpret_current_instruction_without_hook` — correct by construction, no recovered sign/normalize arithmetic to get wrong. Completes the 32-bit long-arithmetic primitive set (mul/unsigned-div/signed-div) the fixed-point 3D renderer sits on.
...
- Data still to decode for the native port: the **`0x5174` menu-def struct**
  ...
  `4201` = the same masked-glyph blit family as the HUD (`skyroads.handrecovered.
